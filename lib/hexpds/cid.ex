defmodule Hexpds.CID do
  ...
  """
  @spec cid(multihash_binary(), Multicodec.multi_codec(), cid_version()) :: {:ok, t()} | {:error, term()}
  def cid(multihash, codec \\ @v0_codec, version \\ @current_version)
  def cid(multihash, codec, version)
      when is_binary(multihash)
           and is_binary(codec)
           and is_integer(version) and version <= @current_version do
  ...
  @spec do_cid(multihash_binary(), Multicodec.multi_codec(), cid_version()) :: {:ok, t()} | {:error, term()}
  ...
  defp do_encode_buffer(%{version: version, codec: codec, multihash: multihash}) when version <= @current_version do
  ...
  defp do_decode(data) do
    with {:ok, {decoded_bin, multibase_codec}} <- Multibase.codec_decode(data),
         ...
         {:ok, cid} <- decode_cid_binary(decoded_bin)
      do
  ...
  defp convert_version(%{version: version} = cid, destination_version) when version == destination_version do
    {:ok, cid}
  end
  ...
  def humanize(cid_string, separator \\ " - ")
    ...
    with true <- String.printable?(separator),
         {:ok, {%{multihash: multihash, codec: codec, version: version}, encoding_id}} <- do_decode(cid_string),
         {:ok, %{digest: digest, name: hash_name}} <- Multihash.decode(multihash) do
      hex_digest = Base.encode16(digest, case: :lower)
      {:ok, [encoding_id, Enum.join(["CIDv", version]), codec, hash_name, hex_digest] |> Enum.join(separator)}
    else
      {:error, _reason} = err -> err
